...
dataset_slug = "lobby"
dataset_name = "Lobby"
dataset_description = "Dynamic registry entry for the local Lobby sample."
...
def load_sample_data(input_path=None):  # noqa: ANN001
    from datosenorden.maintenance.lobby_prototype import load_lobby_sample_payload

    return jsonify(load_lobby_sample_payload(input_path))


def build_entities(payload=None):  # noqa: ANN001
    from datosenorden.maintenance.lobby_prototype import build_lobby_sample_batch

    batch = build_lobby_sample_batch(_null_session(), payload)
