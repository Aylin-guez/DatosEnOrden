...
from datosenorden.maintenance.source_trace import render_source_trace_text
from datosenorden.models import Entity


def _build_parser() -> ArgumentParser:
    parser = ArgumentParser(description="Render a citizen-facing source trace for an entity")
    parser.add_argument("entity_name", help="Entity name to resolve and trace")
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print("No se pudo construir el source trace.", file=sys.stderr)
        print(f"Detalle: {exc}", file=sys.stderr)
        return 1

    print(f"entity: {entity_name} ({entity_type})")
    print(render_source_trace_text(trace))
    return 0
